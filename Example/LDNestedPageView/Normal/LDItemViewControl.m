...
-(id)init
{
    ...
    if(self){
        ...
        _selectedFont = [UIFont systemFontOfSize:18 weight:UIFontWeightBold];
...
@interface LDItemViewControl()
//底部横线
@property(nonatomic,strong)UIView *line;

@end

@implementation LDItemViewControl
...
-(void)setTitleArray:(NSArray *)titleArray
{
    ...
    for (int i=0; i<titleArray.count; i++) {
        ...
        if(i==0){
            ...
            _currentIndex = 0;
            self.line = [[UIView alloc] initWithFrame:CGRectMake(_config.itemWidth*(1-_config.linePercent)/2.0, CGRectGetHeight(self.frame) - _config.lineHieght - _config.lineMarginBottom, _config.itemWidth*_config.linePercent, _config.lineHieght)];
            self.line.hidden = !self.isShowLine;
...
-(void)changeItemColor:(NSInteger)index
{
    for (int i=0; i<_titleArray.count; i++) {
        ...
        if(btn.tag == index+100){
            [btn setTitleColor:_config.selectedColor forState:UIControlStateNormal];
            btn.titleLabel.font = _config.selectedFont;
        }
    }
}
